...
class Expressdent(scrapy.Spider):
    ...
    def _parse_detail(self, response):
        ...
        normal_price = self._get_number(response.css('.summary.entry-summary > .price > span > bdi::text').get())
        internet_price = self._get_number(response.css('.summary.entry-summary > .price > span > bdi::text').get())
